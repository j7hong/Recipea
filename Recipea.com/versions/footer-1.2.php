

<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>Recipes | PHP project - Team PHP</title>
        <meta name="description" content="Homepage for Recipes.com - an community recipe sharing site.">
        <link rel="stylesheet" href="css/styles-1-6.css" /> 
        <!--[if lt IE 9]>
          <script src="//html5shim.googlecode.com/svn/trunk/html5.js"></script>
        <![endif]-->
    </head>
    <body>

<!--        <img src="img/footer/footer-background-colour.png" alt="footer-background-colour" />-->

        <footer class="vertical-down-gradient page-footer">
            <section>
                <h1 class="invisible">Footer icons</h1>
                <img src="img/footer/rss-icon.png" alt="rss-icon" />
                <img src="img/footer/linked-in-icon.png" alt="linked-in-icon" />
                <img src="img/footer/google-plus-icon.png" alt="google-plus-icon" />
                <img src="img/footer/facebook-icon.png" alt="facebook-icon" />
                <img src="img/footer/twitter-icon.png" alt="twitter-icon" />
                <img src="img/footer/youtube-icon.png" alt="youtube-icon" />
            </section>

            <div> <!--style="background-color: #94cb3a; border: 1px dashed red; padding: 0.5%;"-->
                <img src="img/footer/footer-dotted-line.png" al="footer-dotted-line-long" class="dotted-line-xl" />
            </div>

            <ul>
                <br /><br <br /><br />
                <li>www.recipea.com</li>
                <li>&copy; Copyright 2014. Recipea. All rights reserved.</li>               
            </ul>
        </footer> <!--/page-footer-->

    </body>
</html>