<!--- - - - - - - - -  HEADER  - - - - - - - - ---> 
        <link href="css/bootstrap.min.css" rel="stylesheet">
        <link href="css/styles-1-8.css" rel="stylesheet" />
<!--<div class="row">

    <header class="page-header">     

        <div class="col-md-2 col-sm-2 col-xs-11 col-xs-offset-1">
            <a href="index-1.1.php"><img src="img/header/logo.png" alt="Recipea logo" /></a>
        </div>


        <div class="col-md-6 col-sm-6 col-xs-12">
            <nav role="navigation" class="main-nav centre-align">

                <div class="col-md-2 col-sm-2 col-xs-12">
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/home-icon.png" alt="home logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12 clear">
                        <span class="uppercase">Home</span>
                    </div>                                        
                </div>

                <div class="col-md-2 col-sm-12 col-xs-12">
                    <div class="col-md-2 col-sm-12 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-12 col-xs-12 clear">
                        <span class="uppercase">Recipes</span>
                    </div>  
                </div>

                <div class="col-md-2 col-sm-2 col-xs-12">
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12 clear">
                        <span class="uppercase">Recipes</span>
                    </div>  
                </div>


                <a href="#"><img src="img/header/main-nav/events-icon.png" alt="events logo" /></a>
                <span class="uppercase">Events</span>

                <div class="col-md-2 col-sm-2 col-xs-12">
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12 clear">
                        <span class="uppercase">Recipes</span>
                    </div>  
                </div>



                <a href="#"><img src="img/header/main-nav/media-icon.png" alt="media logo" /></a>
                <span class="uppercase">Media</span>

                <div class="col-md-2 col-sm-2 col-xs-12">
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12 clear">
                        <span class="uppercase">Recipes</span>
                    </div>  
                </div>


                <a href="#"><img src="img/header/main-nav/games-icon.png" alt="games logo" /></a>
                <span class="uppercase">Games</span>

                <div class="col-md-2 col-sm-2 col-xs-12">
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12 clear">
                        <span class="uppercase">Recipes</span>
                    </div>  
                </div>

                <a href="#"> <img src="img/header/main-nav/about-icon.png" alt="about logo" /></a>
                <span class="uppercase">About</span>

                <div class="col-md-2 col-sm-2 col-xs-12">
                    <div class="col-md-2 col-sm-2 col-xs-12">
                        <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" /></a>
                    </div>
                    <div class="col-md-2 col-sm-2 col-xs-12 clear">
                        <span class="uppercase">Recipes</span>
                    </div>  
                </div>

                <a href="#"><img src="img/header/main-nav/contact-icon.png" alt="contact logo" /></a>
                <span class="uppercase">Contact</span>

        </div>

      

        </nav>
</div>-->

<div class="row">

    <header class="page-header">     

        <div class="col-md-2"> <!--col-sm-2 col-xs-11 col-xs-offset-1-->
            <a href="#"><img src="img/header/logo.png" alt="Recipea logo" /></a>
        </div>

  <div class="navbar-header navbar-default">
            <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#collapse">
                <span class="sr-only">Toggle Navigation</span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
            </button>

        </div>

        <div class="collapse navbar-collapse col-md-6" id="collapse">
            <ul class="nav navbar-nav">
                <li>
                    <a href="#"><img src="img/header/main-nav/home-icon.png" alt="home logo" />Home</a>
                </li>
                <li>
                    <a href="#"><img src="img/header/main-nav/recipes-icon.png" alt="recipes logo" />Recipes</a>
                </li>
                <li>
                    <a href="#"><img src="img/header/main-nav/events-icon.png" alt="events logo" />Events</a>
                </li>
                <li>
                    <a href="#"><img src="img/header/main-nav/media-icon.png" alt="media logo" />Media</a>
                </li>
                <li>
                    <a href="#"><img src="img/header/main-nav/games-icon.png" alt="games logo" />Games</a>
                </li>
                <li>
                    <a href="#"><img src="img/header/main-nav/about-icon.png" alt="about logo" />About</a>
                </li>
                <li>
                    <a href="#"><img src="img/header/main-nav/contact-icon.png" alt="contact us logo" />Contact Us</a>
                </li>
            </ul>
        </div>
        
<div class="col-md-1">
        <a href="#"><img src="img/header/btn-post-recipe.png" alt="post a recipe button" /></a>                
</div>

<div class="col-md-1">
        <a href="#"><img src="img/header/btn-login.png" alt="login button" /></a>
</div>


</header>    

</div> <!-- /row for header-->

<!--    </body>
</html>-->